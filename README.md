...
**OInspire**（OI + Inspire）不是一个问答机器人。它是一个**有温度的竞赛教练**，运行在 Claude Code 或 Cherry Studio 中。
...
1. 克隆仓库到本地
2. 在 Cherry Studio 中创建 Agent，选择 OInspire 目录作为工作目录
...
OInspire 在 `memory/student/` 下维护分层记忆架构，**回复学生后**按需更新（寒暄类对话不触发）。规则见 `memory/student/INDEX.md`：
